import React from 'react'
import Image from 'next/image';
import Check from '/public/assets/images/shot/check.png';

export const Price = () => {
    return (

        <div className="price my-28">

            <div className="container mx-auto">

                <div className="price-title-section mb-10">
                    <h2 className="price-title text-5xl font-bold text-center">
                        Абанемент
                    </h2>
                </div>

                <div className="price-card-section flex justify-center gap-10">

                    <div className="price-card max-w-sm rounded-2xl overflow-hidden shadow-lg bg-neutral-900 border-2 border-green-500 px-6 flex flex-col justify-center">
                        {/* <img className="w-full" src="/img/card-top.jpg" alt="Sunset in the mountains"> */}

                        <div className="px-2">
                            <h2 className="font-bold text-4xl mb-1 text-center uppercase">НА 3 МЕСЯЦА</h2>
                            <p className="text-center uppercase text-3xl mb-10">
                                1500р
                            </p>
                        </div>

                        <div className="shot-chek-section h-12 flex items-center gap-3 mb-4">

                            <div className="shot-chek-icon">
                                <Image width={1920} height={1080} className='shot-chek-img w-full' src={Check} alt="-_-" />
                            </div>

                            <div className="shot-chek-subtitle-section">
                                <p className="shot-chek-subtitle text-base font-semibold">
                                    Все классы
                                </p>
                            </div>

                        </div>

                        <div className="shot-chek-section h-12 flex items-center gap-3 mb-4">

                            <div className="shot-chek-icon">
                                <Image width={1920} height={1080} className='shot-chek-img w-full' src={Check} alt="-_-" />
                            </div>

                            <div className="shot-chek-subtitle-section">
                                <p className="shot-chek-subtitle text-base font-semibold">
                                    Все мероприятия для участников
                                </p>
                            </div>

                        </div>

                        <div className="shot-chek-section h-12 flex items-center gap-3 mb-4">

                            <div className="shot-chek-icon">
                                <Image width={1920} height={1080} className='shot-chek-img w-full' src={Check} alt="-_-" />
                            </div>

                            <div className="shot-chek-subtitle-section">
                                <p className="shot-chek-subtitle text-base font-semibold">
                                    Полный доступ в тренажерный зал
                                </p>
                            </div>

                        </div>

                        <div className="flex justify-center my-2">
                            <button class="uppercase bg-transparent hover:bg-green-600 text-green-500 font-semibold hover:text-white py-2 px-4 border border-green-500 hover:border-transparent rounded">
                                КУПИТЬ
                            </button>
                        </div>

                    </div>

                    <div className="price-card max-w-sm rounded-2xl overflow-hidden shadow-lg bg-neutral-900 border-2 border-green-500 px-6 flex flex-col justify-center">
                        {/* <img className="w-full" src="/img/card-top.jpg" alt="Sunset in the mountains"> */}

                        <div className="px-2">
                            <h2 className="font-bold text-4xl mb-1 text-center uppercase">НА 3 МЕСЯЦА</h2>
                            <p className="text-center uppercase text-3xl mb-10">
                                1500р
                            </p>
                        </div>

                        <div className="shot-chek-section h-12 flex items-center gap-3 mb-4">

                            <div className="shot-chek-icon">
                                <Image width={1920} height={1080} className='shot-chek-img w-full' src={Check} alt="-_-" />
                            </div>

                            <div className="shot-chek-subtitle-section">
                                <p className="shot-chek-subtitle text-base font-semibold">
                                    Все классы
                                </p>
                            </div>

                        </div>

                        <div className="shot-chek-section h-12 flex items-center gap-3 mb-4">

                            <div className="shot-chek-icon">
                                <Image width={1920} height={1080} className='shot-chek-img w-full' src={Check} alt="-_-" />
                            </div>

                            <div className="shot-chek-subtitle-section">
                                <p className="shot-chek-subtitle text-base font-semibold">
                                    Все мероприятия для участников
                                </p>
                            </div>

                        </div>

                        <div className="shot-chek-section h-12 flex items-center gap-3 mb-4">

                            <div className="shot-chek-icon">
                                <Image width={1920} height={1080} className='shot-chek-img w-full' src={Check} alt="-_-" />
                            </div>

                            <div className="shot-chek-subtitle-section">
                                <p className="shot-chek-subtitle text-base font-semibold">
                                    Полный доступ в тренажерный зал
                                </p>
                            </div>

                        </div>

                        <div className="flex justify-center my-2">
                            <button class="uppercase bg-transparent hover:bg-green-600 text-green-500 font-semibold hover:text-white py-2 px-4 border border-green-500 hover:border-transparent rounded">
                                КУПИТЬ
                            </button>
                        </div>

                    </div>

                    <div className="price-card max-w-sm rounded-2xl overflow-hidden shadow-lg bg-neutral-900 border-2 border-green-500 px-6 flex flex-col justify-center">
                        {/* <img className="w-full" src="/img/card-top.jpg" alt="Sunset in the mountains"> */}

                        <div className="px-2">
                            <h2 className="font-bold text-4xl mb-1 text-center uppercase">НА 3 МЕСЯЦА</h2>
                            <p className="text-center uppercase text-3xl mb-10">
                                1500р
                            </p>
                        </div>

                        <div className="shot-chek-section h-12 flex items-center gap-3 mb-4">

                            <div className="shot-chek-icon">
                                <Image width={1920} height={1080} className='shot-chek-img w-full' src={Check} alt="-_-" />
                            </div>

                            <div className="shot-chek-subtitle-section">
                                <p className="shot-chek-subtitle text-base font-semibold">
                                    Все классы
                                </p>
                            </div>

                        </div>

                        <div className="shot-chek-section h-12 flex items-center gap-3 mb-4">

                            <div className="shot-chek-icon">
                                <Image width={1920} height={1080} className='shot-chek-img w-full' src={Check} alt="-_-" />
                            </div>

                            <div className="shot-chek-subtitle-section">
                                <p className="shot-chek-subtitle text-base font-semibold">
                                    Все мероприятия для участников
                                </p>
                            </div>

                        </div>

                        <div className="shot-chek-section h-12 flex items-center gap-3 mb-4">

                            <div className="shot-chek-icon">
                                <Image width={1920} height={1080} className='shot-chek-img w-full' src={Check} alt="-_-" />
                            </div>

                            <div className="shot-chek-subtitle-section">
                                <p className="shot-chek-subtitle text-base font-semibold">
                                    Полный доступ в тренажерный зал
                                </p>
                            </div>

                        </div>

                        <div className="flex justify-center my-2">
                            <button class="uppercase bg-transparent hover:bg-green-600 text-green-500 font-semibold hover:text-white py-2 px-4 border border-green-500 hover:border-transparent rounded">
                                КУПИТЬ
                            </button>
                        </div>

                    </div>

                </div>

            </div>

        </div>

    )
}
